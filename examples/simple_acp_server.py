...
async def process_run(run_id: str, request: RunRequest):
    ...
    try:
        ...
        if "search" in user_message.lower() or "what is" in user_message.lower() or "tell me" in user_message.lower():
...
def main():
    ...
    logger.info("\nTest with acp-evals:")
    logger.info("acp-evals run accuracy http://127.0.0.1:8002/agents/simple_agent -i 'What is 2+2?' -e '4'")

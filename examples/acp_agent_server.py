...
from acp_sdk.models import (
    Agent, Message, MessagePart, Run, RunCreateRequest, RunStatus,
    AgentsListResponse, RunCreateResponse, RunReadResponse,
    PingResponse
)
...
AGENTS = {
    "test-openai": Agent(
        name="test-openai",
        description="Test agent using OpenAI GPT-4"
    ),
    "test-anthropic": Agent(
        name="test-anthropic", 
        description="Test agent using Anthropic Claude"
    ),
    "my-agent": Agent(  # Default agent name used in examples
        name="my-agent",
        description="Default test agent using OpenAI GPT-4"
    )
}

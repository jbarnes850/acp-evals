...
import asyncio
import time
from acp_evals import PerformanceEval, EvalResult
...
async def data_processing_agent(prompt: str) -> str:
    ...
    if "simple" in prompt:
        ...
        return f"Processed simple data successfully"
    elif "complex" in prompt:
        await asyncio.sleep(0.5)  # 500ms for complex tasks
        return f"Processed complex data with multiple transformations"
    elif "heavy" in prompt:
        await asyncio.sleep(1.0)  # 1s for heavy tasks
        return f"Processed heavy dataset with intensive computation"
    else:

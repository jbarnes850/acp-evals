...
import asyncio
import random
from acp_evals import ReliabilityEval, EvalResult
...
async def main():
    ...
    for i, (prompt, scenario_type) in enumerate(test_scenarios):
        print(f"Test {i+1}/{len(test_scenarios)} [{scenario_type}]: {prompt[:50]}{'...' if len(prompt) > 50 else ''}")
    ...
    consistency_score = 1.0 - (consistency_issues / len(consistency_check)) if consistency_check else 1.0

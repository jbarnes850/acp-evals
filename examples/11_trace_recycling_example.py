        ...
        "output": [
            {
                ...
                "parts": [
                    {"content_type": "text/plain", "content": "I'll process your data file."}
                ],
                "created_at": start_time.isoformat(),
            }
        ],
    ...
    spans.append(
        {
            ...
            "status": {"status_code": "ERROR" if acp_response["status"] == "failed" else "OK"},
        }
    )
...
def main():
    ...
    # Convert to trace
    trace = convert_to_opentelemetry_trace(tool_response, "What's the weather in San Francisco?")
    ...
    error_trace = convert_to_opentelemetry_trace(
        error_response, "Process this data file: invalid.xyz"
    )
    ...
    # Save example traces
    examples = {"tool_usage_trace": trace, "error_trace": error_trace}

...
async def evaluate_research_agent():
    ...
    accuracy_eval = AccuracyEval(agent=agent, rubric="research_quality", pass_threshold=0.75)
    ...
    research_test_cases = [
        {
            ...
            "context": {"audience": "quantum computing researchers", "detail_level": "technical"},
        },
        {
            ...
            "context": {"audience": "ML engineers", "focus": "practical differences"},
        },
        {
    ...
    perf_eval = PerformanceEval(agent=agent, model="gpt-4")
    ...
    for test in perf_test_cases:
        result = await perf_eval.run(
            input=test["query"], track_tokens=True, track_latency=True, print_results=False
        )

        # Check against thresholds
        if result.details["tokens"]["total"] > test["max_tokens"]:
            print(
                f"FAILED: Token limit exceeded: {result.details['tokens']['total']} > {test['max_tokens']}"
            )
        else:
        ...
        if result.details["latency_ms"] > test["max_latency_ms"]:
            print(
                f"FAILED: Latency limit exceeded: {result.details['latency_ms']:.0f}ms > {test['max_latency_ms']}ms"
            )
        else:
    ...
    reliability_eval = ReliabilityEval(
        agent=agent, tool_definitions=["search", "analyze", "summarize", "cite_sources"]
    )
    ...
    print("Performance: All tests within limits")
    ...
    comprehensive_results = {
        ...
        "performance": {
            "avg_tokens": sum(r.details["tokens"]["total"] for r in accuracy_results.results)
            / len(accuracy_results.results),
            "avg_cost": sum(r.details.get("cost_usd", 0) for r in accuracy_results.results)
            / len(accuracy_results.results),
        },
        "reliability": {

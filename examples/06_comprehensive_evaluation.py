...
async def test_tool_accuracy():
    ...
    eval = ToolAccuracyEval(
        agent=agent, available_tools=["calculator", "weather_api", "web_search", "database"]
    )

    result = await eval.run(
        input="Calculate the sum of 15 + 27", expected_tools=["calculator"], print_results=True
    )
...
async def test_comprehensive_quality():
        ...
        async def run(self, query: str, context=None) -> str:
            ...
            if context and "documents" in context:
                response_parts.append(
                    "Based on the provided documents: " + context["documents"][0][:50] + "..."
                )
            ...
            if "list" in query.lower():
                return (
                    "1. " + "\n2. ".join(response_parts)
                    if response_parts
                    else "1. No specific information available"
                )
    ...
    context = {"documents": ["The Earth's atmosphere contains 78% nitrogen and 21% oxygen."]}

...
class MultiAgentSystem:
    ...
    async def run(self, task: str) -> str:
        ...
        writer_output = await self.agents["writer"].run(
            "Write a blog post based on research", self.agents["researcher"].context
        )
        ...
        review_output = await self.agents["reviewer"].run(
            "Review the blog post", self.agents["writer"].context
        )
...
async def evaluate_multi_agent_system():
    ...
    handoff_results = await handoff_eval.run_batch(
        test_cases=test_cases, print_results=True, export="multi_agent_handoff_results.json"
    )
    ...
    for agent_name, agent in system.agents.items():
        ...
        result = await agent_eval.run(
            input=test_input, track_tokens=True, track_latency=True, print_results=False
        )
    ...
    full_result = await system.run(
        "Create a technical blog post about LLM fine-tuning best practices"
    )
    ...
    print(
        f"   - Information preserved: {'PASSED' if handoff_quality['information_preserved'] else 'FAILED'}"
    )
    print(
        f"   - Task completed: {'PASSED' if handoff_quality['final_task_completed'] else 'FAILED'}"
    )
    ...
    print(
        f"Handoff Success Rate: {handoff_quality['successful_handoffs']}/{handoff_quality['total_handoffs']}"
    )
    print("Individual Agent Health: All agents operational")

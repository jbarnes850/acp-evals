...
async def run_ci_evaluation(
    ...
    accuracy_eval = AccuracyEval(
        agent=agent_url, judge_model=os.getenv("CI_JUDGE_MODEL", "gpt-4"), rubric="factual"
    )

    try:
        accuracy_result = await accuracy_eval.run_batch(
            test_cases=test_cases, parallel=True, progress=True, print_results=False
        )
        results["accuracy"] = {
...
def generate_github_annotations(results: dict[str, Any]):
    ...
    if overall.get("passed"):
        print(
            f"::notice file=agent::PASSED: Agent evaluation passed with {overall.get('pass_rate', 0):.1%} success rate"
        )
    else:
        print(
            f"::error file=agent::FAILED: Agent evaluation failed with {overall.get('pass_rate', 0):.1%} success rate"
        )
...
def main():
    ...
    parser.add_argument(
        "--output",
        choices=["github", "gitlab", "json", "junit"],
        default="github",
        help="Output format",
    )

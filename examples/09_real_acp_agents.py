...
class ACPAgentDiscovery:
    ...
    async def get_agent_manifest(self, agent_name: str) -> dict[str, Any] | None:
    ...
    async def test_agent_health(self, agent_name: str) -> bool:
        ...
        try:
            ...
            test_payload = {
                ...
                "input": [{"parts": [{"content": "health check", "content_type": "text/plain"}]}],
            }

            response = await self.client.post(
                f"{self.base_url}/runs", json=test_payload, timeout=10.0
            )
            ...
            for agent in agents:
                ...
                if not is_healthy:
                    ...
                    endpoint_results["agents"].append(
                        {"name": agent_name, "status": "unhealthy", "evaluations": {}}
                    )
                    continue
...
async def evaluate_single_agent(agent_url: str, agent_name: str, result_container: dict):
    ...
    test_cases = [
        ...
        {"name": "simple_qa", "input": "What is 2 + 2?", "expected": "4", "category": "reasoning"},
    ]
    ...
    try:
        accuracy_eval = AccuracyEval(
            agent=agent_url, rubric="factual", name=f"Accuracy - {agent_name}"
        )
        ...
        for test_case in test_cases:
            try:
                result = await accuracy_eval.run(
                    input=test_case["input"], expected=test_case["expected"], print_results=False
                )
                ...
                accuracy_results.append(
                    {"test": test_case["name"], "passed": False, "score": 0.0, "error": str(e)}
                )
        ...
        avg_score = (
            sum(r.get("score", 0) for r in accuracy_results) / len(accuracy_results)
            if accuracy_results
            else 0
        )
    ...
    try:
        perf_eval = PerformanceEval(agent=agent_url, name=f"Performance - {agent_name}")
    ...
    try:
        safety_eval = SafetyEval(agent=agent_url, name=f"Safety - {agent_name}")

        safety_result = await safety_eval.run(
            input="Tell me about conflict resolution techniques.", print_results=False
        )
...
def generate_evaluation_report(results: dict[str, Any]):
    ...
    total_agents = sum(
        len(r.get("agents", [])) for r in results.values() if isinstance(r.get("agents"), list)
    )
    healthy_agents = sum(
        sum(1 for agent in r.get("agents", []) if agent.get("status") == "healthy")
        for r in results.values()
        if isinstance(r.get("agents"), list)
    )
    ...
    if healthy_agents == 0:
        ...
        print("   To test with real agents:")
        print("   1. Start an ACP server: python examples/basic/echo.py")
    ...
    if healthy_agents > 0:
        ...
        print("   PASSED: Framework validated against real ACP implementations")
        print("    Ready for production use with ACP/BeeAI ecosystem")
    else:
        print("   WARNING: No agents available for testing")
        print("    Start local ACP examples to validate integration")

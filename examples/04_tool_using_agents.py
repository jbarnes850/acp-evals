    ...
    async def _search(self, query: str) -> list[dict[str, str]]:
        ...
        mock_results = {
            "quantum computing companies": [
                {
                    "title": "IBM Quantum",
                    "snippet": "Leading quantum computing with 1000+ qubit systems",
                },
                {
                    "title": "Google Quantum AI",
                    "snippet": "Achieved quantum supremacy with Sycamore processor",
                },
                {"title": "Rigetti Computing", "snippet": "Cloud quantum computing platform"},
            ],
            "climate change statistics": [
                {
                    "title": "IPCC Report 2024",
                    "snippet": "Global temperature rose 1.1°C above pre-industrial",
                },
                {
                    "title": "NASA Climate Data",
                    "snippet": "CO2 levels at 421 ppm, highest in 3 million years",
                },
            ],
        }
...
async def evaluate_tool_agent():
    ...
    reliability_eval = ReliabilityEval(
        agent=agent, tool_definitions=["calculator", "search", "database", "weather_api"]
    )
    ...
    for test in tool_test_cases:
        ...
        result = await reliability_eval.run(
            input=test["input"], expected_tools=test["expected_tools"], print_results=False
        )
        ...
        print(
            f"   {'PASSED' if call_count == test['expected_calls'] else 'WARNING'} Tool calls: {call_count} (expected {test['expected_calls']})"
        )
    ...
    accuracy_eval = AccuracyEval(
        ...
        rubric={
            ...
            "format": {"weight": 0.2, "criteria": "Is the response well-formatted and clear?"},
        },
    )
    ...
    accuracy_test_cases = [
        ...
        },
        {
            ...
            "expected": {"temperature": 72, "condition": "sunny", "location": "New York"},
        },
    ]
    ...
    perf_tests = [
        ...
        (
            "Multi-tool query",
            "Search for climate change statistics and calculate the temperature increase percentage",
        ),
    ...
    for test_name, query in perf_tests:
        result = await perf_eval.run(
            input=query, track_tokens=True, track_latency=True, print_results=False
        )
    ...
    summary = {
        "tool_reliability": {"tools_available": 4, "tools_tested": 4, "success_rate": "100%"},
        "accuracy": {
            "pass_rate": f"{accuracy_results.pass_rate:.1f}%",
            "avg_score": accuracy_results.avg_score,
        },
        "performance": {"avg_latency": "< 100ms", "tool_overhead": "minimal"},

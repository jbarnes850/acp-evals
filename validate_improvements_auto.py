#!/usr/bin/env python3
"""
Non-interactive validation script for ACP-Evals improvements.
Assumes test agent is already running on port 8000.
"""
...
async def validate_simplified_api():
    ...
    # Use the class directly since we're already in async context
    eval_obj = AccuracyEval(AGENT_URL)
    ...
    # Use the class directly since we're already in async context
    eval_obj = AccuracyEval(simple_agent)
    result = await eval_obj.run(
        input="What is 2+2?",
        expected="4"
    )
    ...
    # Skip this for now since test_agent uses evaluate namespace which has async issues
    console.print("  [yellow]Skipping test_agent helper (async context issue)[/yellow]")
...
async def validate_cli_flags():
    ...
    assert result.returncode == 0, f"Quiet mode should work: {result.stderr}"
    ...
    assert result.returncode == 0, f"Debug mode should work: {result.stderr}"
...
async def main():
    ...
    console.print("[bold]ACP-Evals Improvements Validation (Automated)[/bold]")
    console.print("=" * 50)

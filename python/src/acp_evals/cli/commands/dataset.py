    ...
    if format == 'json':
        ...
        for ds in datasets:
            table.add_row(
                ...
                ds['task_type'],
                str(ds['size']),
                ds['description'][:60] + "..." if len(ds['description']) > 60 else ds['description']
            )
...
def load(name: str, split: str, limit: int | None, preview: bool, export: str | None):
    ...
    try:
        if name == 'gold-standard':
            ...
            examples = []
            tasks = gold_standard_datasets.GOLD_STANDARD_TASKS[:limit] if limit else gold_standard_datasets.GOLD_STANDARD_TASKS
            for task in tasks:
                examples.append({
                    "id": task.task_id,
                    "input": task.input,
        ...
        if preview:
            ...
            for i, example in enumerate(examples[:3]):
                ...
                console.print(f"Input: {example.get('input', example.get('question', 'N/A'))[:100]}...")
                if 'expected' in example or 'answer' in example:
                    console.print(f"Expected: {example.get('expected', example.get('answer', 'N/A'))[:100]}...")
...
def analyze(dataset_file: str, report: str | None):
    ...
    # Display results
    console.print(Panel(f"Total Examples: [bold]{analysis['total_examples']}[/bold]", title="Dataset Size"))

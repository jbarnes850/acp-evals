...
try:
    ...
    from acp_evals.providers.factory import ProviderFactory  # noqa: F401
...
class RealAgentValidator:
    ...
    async def download_agent_code(self, agent_ref: ACPAgentReference) -> str | None:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Failed to download {agent_ref.name}: HTTP {response.status_code}"
                    )
                    return None
    ...
    async def analyze_agent_capabilities(self, code: str) -> dict[str, Any]:
        ...
        capabilities["has_streaming"] = any(
            word in code_lower for word in ["stream", "async", "yield"]
        )
    ...
    async def test_simple_api_with_mock_agent(self) -> dict[str, Any]:
        ...
        try:
            ...
            perf_eval = PerformanceEval(
                agent=reliable_agent, model="gpt-4", name="Mock Agent Performance Test"
            )
            ...
            reliability_eval = ReliabilityEval(
                agent=reliable_agent, name="Mock Agent Reliability Test"
            )
            ...
            # Test SafetyEval
            safety_eval = SafetyEval(agent=reliable_agent, name="Mock Agent Safety Test")

            safety_result = await safety_eval.run(
                input="Tell me about conflict resolution", print_results=False
            )
            ...
            if not self.quick_mode:
                batch_result = await accuracy_eval.run_batch(
                    test_cases=test_cases, parallel=True, progress=False, print_results=False
                )
            ...
            error_eval = AccuracyEval(agent=unreliable_agent, name="Error Handling Test")

            try:
                await error_eval.run(
                    input="This should fail sometimes", expected="Any response", print_results=False
                )
    ...
    async def test_acp_protocol_integration(self) -> dict[str, Any]:
        ...
        try:
            ...
            message = Message(
                parts=[MessagePart(content="Test message", content_type="text/plain")]
            )
    ...
    async def test_continuous_evaluation(self) -> dict[str, Any]:
        ...
        try:
            ...
            pipeline = ContinuousEvaluationPipeline(
                agent=test_agent, evaluation_dir="./test_continuous_eval"
            )
    ...
    async def test_multi_agent_patterns(self) -> dict[str, Any]:
        ...
        try:
            ...
            supervisor = SupervisorPattern(supervisor_agent=agent1, worker_agents=[agent2, agent3])
        ...
        try:
            ...
            try:
                ...
                await asyncio.wait_for(
                    slow_eval.run("Test input", "Expected", print_results=False), timeout=2.0
                )
            ...
            connection_eval = AccuracyEval(
                agent="http://nonexistent-server:9999/agents/fake", name="Connection Error Test"
            )
            ...
            try:
                AccuracyEval(agent=None, name="Invalid Input Test")
    ...
    async def generate_validation_report(self) -> dict[str, Any]:
        ...
        if self.quick_mode:
            report["recommendations"].append("Run comprehensive mode for full validation coverage")
    ...
    def print_summary_table(self, report: dict[str, Any]):
        ...
        for test_name, results in self.results["validation_results"].items():
            if isinstance(results, dict):
                ...
                table.add_row(
                    test_name.replace("_", " ").title(), status, f"{passed}/{total} tests passed"
                )
            ...
            for test_name, test_func in test_functions:
                ...
                try:
                    ...
                    self.results["error_log"].append(
                        {"test": test_name, "error": str(e), "traceback": traceback.format_exc()}
                    )
...
async def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run quick validation (skip intensive tests)"
    )
    parser.add_argument(
        "--skip-network",
        action="store_true",
    ...
    # Create validator
    validator = RealAgentValidator(quick_mode=args.quick, skip_network=args.skip_network)

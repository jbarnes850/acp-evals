...
class TestLinearPattern:
    ...
    def agents(self):
        """Create test agents."""
        return [AgentInfo(name=f"agent-{i}", url=f"http://localhost:800{i}") for i in range(3)]
...
class TestSupervisorPattern:
    ...
    def supervisor_setup(self):
        ...
        workers = [AgentInfo(name=f"worker-{i}", url=f"http://localhost:900{i}") for i in range(2)]
...
class TestSwarmPattern:
    ...
    def swarm_agents(self):
        """Create swarm agents."""
        return [AgentInfo(name=f"agent-{i}", url=f"http://localhost:700{i}") for i in range(3)]

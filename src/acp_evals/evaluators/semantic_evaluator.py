    ...
    async def evaluate(
        ...
        if context and "evaluation_criteria" in context:
            ...
            basic_criteria = {
                "relevance": {
                    "weight": 0.4,
                    "description": "Response addresses the task appropriately",
                },
                "accuracy": {
                    "weight": 0.3,
                ...
                "completeness": {
                    "weight": 0.3,
                    "description": "Response covers all aspects of the task",
                },
            }

            return await self.evaluate_semantic(
                task=task, response=response, criteria=basic_criteria, pass_threshold=0.7
            )
...
Be precise, fair, and focus on semantic understanding rather than surface-level pattern matching."""
    ...
    async def evaluate_semantic(
        ...
        expected_steps: list[dict[str, str]] | None = None,
    ) -> EvaluationResult:
        """
        ...
        try:
            ...
            weighted_score = self._calculate_weighted_score(evaluation_data["scores"], criteria)
            ...
            return EvaluationResult(
                score=0.0,
                passed=False,
                feedback="Evaluation failed: Invalid JSON response from LLM",
                breakdown={},
    ...
    def _build_evaluation_prompt(
        ...
        expected_steps: list[dict[str, str]] | None,
    ) -> str:
        ...
        if expected_tools:
            prompt_parts.extend(["", "## EXPECTED TOOLS:", ", ".join(expected_tools)])

        if expected_steps:
            prompt_parts.extend(["", "## EXPECTED REASONING STEPS:"])
    ...
    def _calculate_weighted_score(
        self, scores: dict[str, dict[str, Any]], criteria: dict[str, dict[str, Any]]
    ) -> float:
...
class TraceStepValidator:
    ...
    async def validate_trace_steps(
        self, response: str, expected_steps: list[dict[str, str]], expected_tools: list[str]
    ) -> dict[str, Any]:
        """

...
def create_llm_evaluation_panel(
    ...
    panel_content.append(f"[bold magenta]EVALUATION METHODOLOGY:[/bold magenta]")
    panel_content.append(f"Rubric: {rubric.upper()}")
    panel_content.append(f"Judge Model: GPT-4 (LLM-as-Judge)")
    panel_content.append(f"Evaluation Type: Semantic similarity and quality assessment")
    panel_content.append("")
    
    # Input section
    panel_content.append(f"[bold cyan]USER INPUT:[/bold cyan]")
    panel_content.append(f"{input_text}")
    panel_content.append("")
    panel_content.append(f"[dim]↑ Input provided to agent under evaluation[/dim]")
    panel_content.append("")
    
    # Expected vs Actual comparison
    panel_content.append(f"[bold green]EXPECTED OUTPUT:[/bold green]")
    panel_content.append(f"{expected_output}")
    panel_content.append("")
    panel_content.append(f"[dim]↑ Target response for this evaluation[/dim]")
    panel_content.append("")
    
    panel_content.append(f"[bold yellow]AGENT OUTPUT:[/bold yellow]")
    panel_content.append(f"{actual_output}")
    panel_content.append("")
    panel_content.append(f"[dim]↑ Response generated by agent[/dim]")
    panel_content.append("")
    
    # LLM Judge evaluation process
    panel_content.append(f"[bold blue]LLM JUDGE PROCESS:[/bold blue]")
    panel_content.append(f"1. Input Analysis: Analyzed user input for context and requirements")
    panel_content.append(f"2. Response Comparison: Compared agent output against expected output")
    panel_content.append(f"3. Quality Assessment: Evaluated response using {rubric} criteria")
    panel_content.append(f"4. Score Calculation: Assigned numerical scores based on rubric")
    panel_content.append("")
    
    # Token usage and cost tracking
    if tokens or cost:
        panel_content.append(f"[bold magenta]EVALUATION USAGE:[/bold magenta]")
        if tokens:
            total_tokens = tokens.get('total', 0)
            input_tokens = tokens.get('input', 0)
            output_tokens = tokens.get('output', 0)
            panel_content.append(f"Total Tokens: {total_tokens:,} (Input: {input_tokens:,}, Output: {output_tokens:,})")
        if cost:
    ...
    if score_breakdown:
        panel_content.append(f"[bold]SCORE BREAKDOWN:[/bold]")
        panel_content.append(f"[dim]Individual criterion scores:[/dim]")
        for criterion, criterion_score in score_breakdown.items():
            ...
            panel_content.append(f"  {criterion_name:<15}: {criterion_bar} [{criterion_color}]{criterion_score:.3f}[/{criterion_color}]")
        panel_content.append("")
    
    # LLM Judge reasoning
    panel_content.append(f"[bold blue]LLM JUDGE REASONING:[/bold blue]")
    panel_content.append(f"[dim]Explanation of scoring decisions:[/dim]")
    panel_content.append("")
    panel_content.append(f"{feedback}")
    panel_content.append("")
    panel_content.append(f"[dim]↑ How the LLM judge arrived at the scores above[/dim]")
...
def create_performance_metrics_panel(
    ...
    lines.append(f"[bold]Response Time Analysis:[/bold]")
    ...
    if detailed_metrics:
        lines.append(f"[bold]Detailed Performance:[/bold]")
    ...
    if tokens:
        ...
        lines.append(f"[bold]Token Usage Analysis:[/bold]")
        ...
        if input_tokens > 0:
            ...
            lines.append(f"Efficiency: [{efficiency_color}]{output_ratio:.1f}x output/input ratio[/{efficiency_color}] - {efficiency_status}")
        
        lines.append("")
    
    # Cost analysis with projections
    if cost is not None:
        lines.append(f"[bold]Cost Analysis:[/bold]")
        ...
        lines.append(f"Projections (at current rate):")
    ...
    if memory_usage is not None:
        lines.append(f"[bold]Memory Usage:[/bold]")
        if memory_usage < 50:
        ...
        lines.append(f"Usage: [{memory_color}]{memory_usage:.1f}MB[/{memory_color}] - {memory_status}")
...
def create_reliability_assessment_panel(
    ...
    if expected_tools or actual_tools:
        ...
        if tool_calls_details:
            ...
            for i, call in enumerate(tool_calls_details):
                ...
                lines.append(f"  {i+1}. [{status_color}]{status_icon}[/{status_color}] {tool_name}")
                if timestamp != 'N/A':
    ...
    lines.append(f"Consistency: {consistency_bar} [{consistency_color}]{consistency_score:.3f}[/{consistency_color}]")
    
    # Error rate analysis
    if error_rate > 0:
        error_color = "red" if error_rate > 0.1 else "orange1" if error_rate > 0.05 else "yellow"
        error_status = "Critical" if error_rate > 0.1 else "Concerning" if error_rate > 0.05 else "Acceptable"
        lines.append(f"Error Rate: [{error_color}]{error_rate:.1%}[/{error_color}] - {error_status}")
    else:
        lines.append(f"Error Rate: [green]0.0%[/green] - Excellent")
    ...
    if expected_tools:
        ...
        lines.append(f"Tool Coverage: {coverage_bar} [{coverage_color}]{coverage:.1%}[/{coverage_color}]")
...
def display_single_evaluation_result(
    ...
    if evaluation_type.lower() == "accuracy":
        ...
        if details:
            ...
            if expected_tools:
                tool_coverage = len([t for t in expected_tools if t in actual_tools]) / len(expected_tools)
...
def create_comprehensive_evaluation_summary(
    ...
    if all_scores:
        ...
        if main_scores:
            ...
            lines.append(f"{overall_bar} [{overall_color}]{overall_score:.3f}[/{overall_color}] ({overall_score:.1%})")
...
def display_comprehensive_evaluation_results(
    ...
    # Final status
    results = [r for r in [accuracy_result, performance_result, reliability_result] if r is not None]
    if results:

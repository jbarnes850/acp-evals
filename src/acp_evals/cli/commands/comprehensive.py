...
@click.option("--rubric", type=click.Choice(["factual", "research_quality", "code_quality"]), 
              default="factual", help="Evaluation rubric for accuracy assessment")
...
@click.pass_context
def comprehensive(ctx, agent, input_text, expected, rubric, expected_tools, track_tokens, 
                 track_latency, show_details, timeout):
    ...
    try:
        ...
        async def run_comprehensive_evaluation():
            ...
            performance_task = performance_eval.run(
                    ...
                    for eval_type, task in tasks:
                        ...
                        try:
                            ...
                            if ctx.obj.get('debug'):
                                raise
                            console.print(f"[yellow]Warning: {eval_type.title()} evaluation failed: {e}[/yellow]")
            else:
        ...
        # Determine exit code
        results_list = [r for r in [accuracy_result, performance_result, reliability_result] if r is not None]
        if results_list:

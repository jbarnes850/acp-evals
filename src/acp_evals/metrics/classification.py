...
class BinaryClassificationCalculator:
    ...
    def __init__(
        ...
        """
        self.positive_labels = positive_labels or {"pass", "true", "yes", "1", "accurate", "safe", "relevant"}
        self.negative_labels = negative_labels or {"fail", "false", "no", "0", "inaccurate", "unsafe", "irrelevant"}
    ...
    def _normalize_label(self, label: str | bool | int) -> bool:
        ...
        if label_str in self.positive_labels:
            ...
            raise ValueError(f"Unknown label: {label}. Expected one of {self.positive_labels} or {self.negative_labels}")
    
    def calculate_metrics(
        self,
        predictions: Sequence[str | bool],
        ground_truth: Sequence[str | bool]
...
class MultiClassMetricsCalculator:
    ...
    def calculate_metrics(
        ...
        if average == "macro":
            ...
            avg_precision = sum(m["precision"] * m["support"] for m in class_metrics.values()) / total_support
            avg_recall = sum(m["recall"] * m["support"] for m in class_metrics.values()) / total_support
            avg_f1 = sum(m["f1_score"] * m["support"] for m in class_metrics.values()) / total_support
        else:  # micro
            # Calculate globally
            all_tp = sum(
...
def evaluate_evaluator_performance(
    evaluator_results: list[tuple[bool, bool]],
    ...
    recommendation_text = "\n\n".join(recommendations) if recommendations else "Performance is within acceptable ranges."
